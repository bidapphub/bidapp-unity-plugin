...
namespace UnityEditor.XCodeEditor
{
	public class PBXBuildFile : PBXObject
	{
		...
		private const string EMBED_CODE_SIGN = "CodeSignOnCopy";
		private const string EMBED_REMOVE_HEADERS = "RemoveHeadersOnCopy";
		...
		public bool SetEmbed(bool embed = false)
		{
			...
			if (!_data.ContainsKey(SETTINGS_KEY))
			{
				if (embed)
				{
					...
					attributes.Add(EMBED_CODE_SIGN);
					attributes.Add(EMBED_REMOVE_HEADERS);
			...
			{
				attributes.Remove(EMBED_CODE_SIGN);
				attributes.Remove(EMBED_REMOVE_HEADERS);
			}
