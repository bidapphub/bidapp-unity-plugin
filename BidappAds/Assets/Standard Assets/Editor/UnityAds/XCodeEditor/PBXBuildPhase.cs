...
//			Debug.Log( "constructor child" + GetType().Name );
	...
	{
		private const string DST_SUBFOLDER_SPEC_KEY = "dstSubfolderSpec";
		public uint GetDstSubfolderSpec()
		{
			if (this.ContainsKey(DST_SUBFOLDER_SPEC_KEY))
            {
				return (uint)(int)this.ValueForKey(DST_SUBFOLDER_SPEC_KEY);
            }
			else
            {
				return 0;
