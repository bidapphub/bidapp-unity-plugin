...
//        return results
        ...
        private static string embeddedFrameworkScript = "APP_PATH=\\\"${TARGET_BUILD_DIR}/${WRAPPER_NAME}\\\"\n" +
        "\n" +
        ...
        public void AddFrameworkStrippingScript()
        {
            ArrayList existingShellScriptGuids = new ArrayList();
            foreach( KeyValuePair<string, PBXShellScriptBuildPhase> currentObject in shellScriptBuildPhases ) {
                PBXShellScriptBuildPhase shellScript = currentObject.Value;
                
                string shellScriptString = (string)shellScript.data["shellScript"];
                if (shellScriptString == embeddedFrameworkScript)
                {
                    Debug.Log( "Find exisiting FW stripping script: " + currentObject.Key);
                    existingShellScriptGuids.Add(currentObject.Key);
                }
            }
            
            foreach( KeyValuePair<string, PBXNativeTarget> currentObject in nativeTargets ) {
                
                PBXNativeTarget pn = currentObject.Value;
                
                string productType = (string)pn.data["productType"];
                
                if (null != productType &&
                productType.EndsWith("unit-test"))
                {
                    continue;
                }
                
                ArrayList buildPhases = (ArrayList)pn.data["buildPhases"];
                
                if (null != buildPhases &&
                0 != buildPhases.Count)
                {
                    string lastPhaseGuid = (string)buildPhases[buildPhases.Count-1];
                    
                    if (!existingShellScriptGuids.Contains(lastPhaseGuid))
                    {
                        PBXDictionary pd = new PBXDictionary();
                        pd.Add("isa", "PBXShellScriptBuildPhase");
                        pd.Add("buildActionMask", 2147483647);
                        pd.Add("files", new ArrayList());
                        pd.Add("inputPaths", new ArrayList());
                        pd.Add("outputPaths", new ArrayList());
                        pd.Add("runOnlyForDeploymentPostprocessing", 0);
                        pd.Add("shellPath", "/bin/sh");
                        pd.Add("shellScript", embeddedFrameworkScript);
                        
                        string guid__ = PBXObject.GenerateGuid();
                        shellScriptBuildPhases.Add(new PBXShellScriptBuildPhase(guid__, pd));
                        
                        buildPhases.Add(guid__);
                        
                        pn.Remove("buildPhases");
                        pn.Add("buildPhases", buildPhases);
                    }
                    else
                    {
                        Debug.Log( "FW stripping script already exists: " + lastPhaseGuid );
                    }
                }
            }
        }
...
//		#endregion
		...
		{	
			...
			//Embedded BidappAds.framework should be added to "PBXCopyFilesBuildPhase"
			PBXFileReference bidappFRef = this.GetFile("BidappAds.framework");
			if (null != bidappFRef)
			{
				foreach (KeyValuePair<string, PBXCopyFilesBuildPhase> currentObject in copyBuildPhases)
				{
					if (currentObject.Value.GetDstSubfolderSpec() == 10)
					{
						PBXBuildFile buildFile = new PBXBuildFile(bidappFRef, false, true);
						buildFiles.Add(buildFile);
						currentObject.Value.AddBuildFile(buildFile);
					}
